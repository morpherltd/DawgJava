...
class LevelBuilder {
    static <TPayload> void buildLevelsExcludingRoot(Node<TPayload> root) {
        ArrayList<HashMap<NodeWrapper<TPayload>, NodeWrapper<TPayload>>> levels = new ArrayList<HashMap<NodeWrapper<TPayload>, NodeWrapper<TPayload>>>();
        Deque<StackNode<TPayload>> stack = new ArrayDeque<StackNode<TPayload>>();
            ...
            if (stack.peek().ChildIterator.hasNext()) {
                ...
                if (stack.size() > 0) {
                    ...
                    if (levels.size() <= level) {
                        levels.add(new HashMap<NodeWrapper<TPayload>, NodeWrapper<TPayload>>());
                    }

                    HashMap<NodeWrapper<TPayload>, NodeWrapper<TPayload>>
                        dictionary = levels.get(level);

                    NodeWrapper<TPayload> nodeWrapper = new NodeWrapper<TPayload>(
...
//        System.out.println("Stop");
    ...
    private static <TPayload> void push(Deque<StackNode<TPayload>> stack,
                                        Node<TPayload> node) {
        StackNode<TPayload> sn = new StackNode<TPayload>();
        sn.Node = node;
